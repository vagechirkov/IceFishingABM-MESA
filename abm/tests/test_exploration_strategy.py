...
def random_walker():
    np.random.seed(0)
    return RandomWalkerExplorationStrategy(grid_size=10, mu=1.5, dmin=1.0, L=5.0, alpha=0.1)
...
def test_choose_destination_trivial(exploration_strategy):
    ...
    destination = exploration_strategy.choose_destination(np.empty((0, 2)), np.empty((0, 2)), np.empty((0, 2)))
    x, y = destination
...
def test_invalid_inputs(exploration_strategy):
    ...
    with pytest.raises(AssertionError):
        exploration_strategy.choose_destination(np.array([1, 2]), np.empty((0, 2)), np.empty((0, 2)))

    with pytest.raises(AssertionError):
        exploration_strategy.choose_destination(np.empty((0, 2)), np.array([[1, 2, 3]]), np.empty((0, 2)))
...
def test_adjust_for_social_cue(random_walker):
    ...
    np.random.seed(1)
    adjusted_destination = random_walker._adjust_for_social_cue(np.array([4, 4]), social_locs)
    
    # Check if it switched to the nearest social cue or stayed at original position
    assert (np.array_equal(adjusted_destination, [5, 5]) or 
            np.array_equal(adjusted_destination, [4, 4]))
